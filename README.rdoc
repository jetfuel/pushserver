...
PusServer Gem only has one method
  sendPushToUser(uid, alert)
    * uid: uid is the username/user_email
    * alert: The message that will be sent to the user. 
    * return : boolean
...
PushServer.sendPushToUser("uer's email", "This is the message")
============
